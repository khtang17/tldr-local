...
def bb_filter(bb_smiles, inSMARTS, exSMARTS, hac_range=None):
    ...
    if inclusion_rules_check(cmpd_mol, inSMARTS) and exclusion_rules_check(cmpd_mol, exSMARTS):
        if hac_range:
            if filter_by_hac(cmpd_mol, hac_range):
                return True
            else:
                return False
        else:
...
def main():
    ...
    # check if input is file or list
    if isinstance(args.bb_file, list):
        input_list = args.bb_file
    else:
